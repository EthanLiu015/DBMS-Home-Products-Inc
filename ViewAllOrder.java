...
public class ViewAllOrder {
    ...
    addSection(detailsPanel, "Order Information", createOrderInfoPanel());
    addSection(detailsPanel, "Customer Information", createContactInfoPanel());
    addSection(detailsPanel, "Payment Information", createPaymentInfoPanel());
    addSection(detailsPanel, "Product Information", createProductInfoPanel());
    addSection(detailsPanel, "Shipping Information", createShippingInfoPanel());
...
private JPanel createOrderInfoPanel() {
    ...
    addField(panel, "Order ID: ", String.valueOf(order.getOrderID()));
    addField(panel, "Order Date: ", String.valueOf(order.getOrderDate()));
    addField(panel, "Status: ", order.getStatus());
...
private JPanel createContactInfoPanel() {
    ...
    addField(panel, "Customer ID: ", String.valueOf(order.getCustomerID()));
    addField(panel, "Customer Name: ", order.getCustomerFirstName() + " " + order.getCustomerLastName());
...
private JPanel createPaymentInfoPanel() {
    ...
    if (uniquePayments.isEmpty()) {
        addField(panel, "Payments: ", "No payments recorded for this order");
        return panel;
    }
    ...
    for (Order.Payment payment : uniquePayments.values()) {
        ...
        if (count > 1) {
            ...
            addField(panel, "Payment " + count, "-------------------");
        } else {
            addField(panel, "Payment 1", "-------------------");
        }
        
        // Add payment details
        addField(panel, "Payment ID: ", String.valueOf(payment.getPaymentID()));
        addField(panel, "Payment Date: ", String.valueOf(payment.getPaymentDate()));
        addField(panel, "Card Holder: ", payment.getCardHolder());
        addField(panel, "Payment Method: ", payment.getPaymentMethod());
        addField(panel, "Amount: ", String.format("$%.2f", payment.getAmount()));
        addField(panel, "Credit Card: ", payment.getIsCreditCard() ? "Yes" : "No");
    ...
    panel.add(Box.createVerticalStrut(20));
    addField(panel, "Total Amount Paid: ", String.format("$%.2f", totalPaid));
...
private JPanel createProductInfoPanel() {
    ...
    if (items.isEmpty()) {
        addField(panel, "Products: ", "No products in this order");
        return panel;
    }
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        if (i > 0) {
            panel.add(Box.createVerticalStrut(20));
            addField(panel, "Product " + (i + 1), "-------------------");
        } else {
            addField(panel, "Product 1", "-------------------");
        }
        
        // Add product details
        addField(panel, "Product ID: ", item.getProductID());
        addField(panel, "Description: ", item.getProductDescription());
        addField(panel, "Quantity: ", String.valueOf(item.getQuantityOrdered()));
        addField(panel, "Price per Unit: ", String.format("$%.2f", item.getQuotedPrice()));
        addField(panel, "Subtotal: ", String.format("$%.2f", item.getQuotedPrice() * item.getQuantityOrdered()));
    }
...
private JPanel createShippingInfoPanel() {
    ...
    addField(panel, "Shipping Date: ", String.valueOf(order.getShippingDate()));
    addField(panel, "Shipping Method: ", order.getShippingMethod());
...
private JPanel createFinancialSummaryPanel() {
    ...
    double discountPercentage = (discount / regularTotal) * 100;
    
    addField(panel, "Unit Price Total: ", String.format("$%.2f", regularTotal));
    addField(panel, "Discount Savings: ", String.format("$%.2f (%.0f%%)", discount, discountPercentage));
    addField(panel, "Subtotal (Quoted Price): ", String.format("$%.2f", subtotal));
    addField(panel, "Sales Tax Rate: ", String.format("%.0f%%", order.getSalesTax() * 100));
    addField(panel, "Tax Amount: ", String.format("$%.2f", tax));
    addField(panel, "Final Total: ", String.format("$%.2f", total));
